...
function PizzaItem({ id, name, price, imageUrl, sizes, types }) {
  const [pizzaSize, setPizzaSize] = useState(0);
  const [pizzaType, setPizzaType] = useState(0);
  const dispatch = useDispatch();
  const thisPizzaCount = useSelector((state) => state.cart.items.find((obj) => obj.id === id));

  const nameTypes = ['тонкое', 'толстое'];

  const addedCount = thisPizzaCount ? thisPizzaCount.counter : 0;
      ...
      <div className="pizza-block">
        ...
        <div className="pizza-block__bottom">
          ...
          <div onClick={onClickAdd} className="button button--outline button--add">
            ...
            <span>Добавить</span>
            {addedCount > 0 && <i>{thisPizzaCount.counter}</i>}
          </div>
        </div>
      </div>
    </div>
